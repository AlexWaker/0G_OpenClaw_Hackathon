...
export async function launchOpenClawChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`☪️ openclaw browser profile decorated (${profile.color})`);
    } catch (err) {
  ...
  log.info(
    `☪️ openclaw browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
