...
const restoreZeroGMemorySyncIfAvailableMock = vi.hoisted(() =>
  vi.fn(
    async (): Promise<ZeroGMemorySyncRestoreResult> => ({
      skipped: true,
...
vi.mock("../zerog/storage-sync.service.js", () => ({
  restoreZeroGMemorySyncIfAvailable: restoreZeroGMemorySyncIfAvailableMock,
}));
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeEach(() => {
    ...
    restoreZeroGMemorySyncIfAvailableMock.mockClear();
  });
  ...
  it("restores existing 0G memory during local setup and exposes it in JSON output", async () => {
    await withStateDir("state-zerog-restore-json-", async (stateDir) => {
      restoreZeroGMemorySyncIfAvailableMock.mockResolvedValueOnce({
        skipped: false,
        restoredCount: 2,
        skippedCount: 0,
        failedCount: 1,
      ...
      expect(restoreZeroGMemorySyncIfAvailableMock).toHaveBeenCalledWith(
        expect.objectContaining({
          skipWhenLocalFilesPresent: true,
        }),
      );
      expect(ensureWorkspaceAndSessionsMock.mock.invocationCallOrder[0]).toBeLessThan(
        restoreZeroGMemorySyncIfAvailableMock.mock.invocationCallOrder[0] ??
      ...
      expect(parsed.zeroGMemoryRestore).toEqual({
        status: "synced",
        ...
        manifestRootHash: "0xmanifest",
        message:
          "Detected existing 0G memory for this wallet and synced 2 files locally. 1 file failed and will retry on startup.",
      });
    });
  }, 60_000);

  it("logs that existing 0G memory was already synced during local setup", async () => {
    await withStateDir("state-zerog-restore-text-", async (stateDir) => {
      restoreZeroGMemorySyncIfAvailableMock.mockResolvedValueOnce({
        skipped: false,
        restoredCount: 1,
        skippedCount: 0,
      ...
      const runtimeWithLogs: RuntimeEnv = {
        ...runtime,
        log: vi.fn(),
      };
      ...
      const logLines = vi.mocked(runtimeWithLogs.log).mock.calls.map(([line]) => String(line));
      expect(logLines).toContain(
        "Detected existing 0G memory for this wallet and synced 1 file locally.",
      );
    });
  }, 60_000);
