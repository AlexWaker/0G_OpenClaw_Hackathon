...
export async function startGatewaySidecars(params: {
  ...
  try {
    ...
    params.log.warn(`0G memory restore on startup failed: ${String(err)}`);
  }

  void startZeroGMemorySyncService({
    cfg: params.cfg,
    log: {
    ...
    params.log.warn(`0G memory sync startup failed: ${String(err)}`);
  });
