...
export async function runSetupWizard(
  ...
  */
  ...
  const { restoreZeroGMemorySyncIfAvailable } = await import("../zerog/storage-sync.service.js");
  try {
    ...
    if (restoreResult.restoredCount > 0) {
      ...
      const failureSuffix =
        restoreResult.failedCount > 0
          ? ` ${restoreResult.failedCount} file${restoreResult.failedCount === 1 ? "" : "s"} failed and will retry on startup.`
          : "";
      await prompter.note(
        `Restored ${restoreResult.restoredCount} 0G memory ${restoredLabel} for this wallet.${failureSuffix}`,
        "0G memory",
      );
    } else if (restoreResult.failedCount > 0) {
      await prompter.note(
        `0G memory restore could not complete. ${restoreResult.failedCount} file${restoreResult.failedCount === 1 ? "" : "s"} failed and will retry on startup.`,
        "0G memory",
      );
    }
  } catch (err) {
    await prompter.note(`0G memory restore failed: ${String(err)}`, "0G memory");
  }
